...
.controller('eseguiIstanzaCtrl', 
  ...
  $scope.inviaDati = function() {
    ...
    Upload.upload({
        ...
        $scope.iC.statoIstanza = 201;

        if(response.data) {
            $scope.iC.errorMsg= response.data.msg;
            $scope.iC.errorTitle= response.data.title;
            $scope.iC.successTxtMsg= 'Error status: ' + response.status;
        } else {
            $scope.iC.errorMsg= "Errore segnalato agli amministratori di sistema. Riprovare più tardi.";
            $scope.iC.errorTitle= "Errore generico";
            $scope.iC.successTxtMsg= 'Error status: ' + response.status;
        }
