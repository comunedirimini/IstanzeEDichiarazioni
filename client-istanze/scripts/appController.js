...
.controller("AppCtrl", 
        ...
        $log.info("AppCtrl start ... show ENV vars");
        $log.info(ENV);
        ...
        $log.info('AppCtrl host:', host); 

        if( (host == 'localhost') || (host == '10.10.6.38') ) {
            $rootScope.base_url = 'http://' + host + ':8009';
        } else {
        ...
        $log.info('AppCtrl check if JWT exist ...');
        if ($localStorage.JWT){
            $log.info('AppCtrl reload JWT in http header');
        ...
        $log.info('AppCtrl rootScope Url: '+ $rootScope.base_url);
